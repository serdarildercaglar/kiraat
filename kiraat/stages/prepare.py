...
class PrepareStage(SourceStage):
    ...
    def process_source(self, source: Mapping[str, Any]) -> Sequence[Mapping[str, Any]]:
        ...
        filters = [f"highpass=f={float(self.opts.get('highpass_hz', 40.0))}",
                   f"alimiter=limit={10 ** (float(self.opts.get('peak_ceiling_db', -1.0)) / 20):.4f}"]
        ...
        if info["duration"] and decoded < float(self.opts.get("truncated_ratio", 0.95)) * info["duration"]:
