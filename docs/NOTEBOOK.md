...
8 Eylül 2026, `scripts/probe_sentence_cuts.py`. Ablasyon 206 kayıtlık
...
Betik önerilen alt kümeyi `export` ile birebir aynı kurar: `duplicate`
işareti veritabanında durmaz, dışa aktarımda `dedupe.mark_duplicates` ile
konur, bu yüzden betik yinelemeleri (576 klip) kendisi işaretleyip
politikayı ondan sonra uygular. Sayılar veri kartındaki ve `run.json`'daki
alt kümeyle örtüşür.

| küme | klip | saat | baş kırığı | son kırığı |
|---|---|---|---|---|
| tüm korpus | 1.840.404 | 3.105,7 | 27.483 (%1,49) | 33.346 (%1,81) |
| önerilen alt küme | 1.547.494 | 2.575,2 | **23 (%0,0015)** | 430 (%0,03) |
| politika dışı | 292.910 | 530,5 | 27.460 (%9,37) | 32.916 (%11,24) |

Sayılar tek başına değil, işaretle birlikte okunur. Korpustaki kırıkların
neredeyse tamamı hattın kendi işaretlediği kliplerde toplanıyor (bir klip
birden çok işaret taşıyabilir; "işaretsiz" ayrık kümedir):

| işaret | klip | baş kırığı | son kırığı |
|---|---|---|---|
| işaretsiz | 1.558.409 | 23 (%0,00) | 521 (%0,03) |
...
| `duplicate` | 576 | 45 (%7,81) | 65 (%11,28) |

Baş kırıklarının %92,3'ü `forced_split` taşıyor; `gap_split` ile birlikte
sayıldığında pay **%97,6** (27.483 kırığın 26.837'si). Bunlar hattın
"burada cümle sınırı bulamadım, tavana ya da boşluğa dayanarak kestim"
dediği kliplerdir. Yani kırık, hattın göremediği bir kusur değil, kendi
ilan ettiği bir kusurdur — ve ilan edildiği için politika onu önerilen alt
kümenin dışında tutuyor. Değişmezin ölçülmüş hâli budur: işaretsiz
1.558.409 klipte cümle ortasından başlayan **23 klip** var, yüz binde 1,5.
